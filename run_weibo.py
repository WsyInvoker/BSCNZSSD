...
from utils.data_utils import  Tokenizer4Bert, weiboDataset
...
class Instructor(object):
    def __init__(self,opt):
        ...
        self.trainset = weiboDataset(fname=self.opt.train_dir,tokenizer=tokenizer)
        self.valset = weiboDataset(fname=self.opt.dev_dir, tokenizer=tokenizer)
        self.testset = weiboDataset(fname=self.opt.test_dir,tokenizer=tokenizer)
    ...
    def train_traditon(self,writer1):
        ...
        for i_epoch in range(self.opt.epochs):
            ...
            for i_batch, batch in enumerate(train_loader):
                ...
                input_features = [batch[feat_name].to(self.opt.device) for feat_name in self.opt.input_features]
                index = batch['index']          #index是数据集中的样本编号
                # print("$$$$index: ",index)
                true_stance = batch['polarity']         #true_stance是一个15长度的中文单词列表,下面新增转化为张量(Tensor)格式
                # print("@@@@true_stance: ",true_stance)

                # true_stance = text_labels_to_tensor(true_stance) ###
...
if __name__ == "__main__":
    ...
    polarities = {
        'sem16_naacl': [0, 1, 2],   # 0 for Against, 1 for Favor, 2 for none 难道不是0反对1无关2喜欢吗

...
class weiboDataset(Dataset):
    def numerical_to_textual_labels(labels_list, polarity_mapping):
        return [polarity_mapping[label] for label in labels_list]
    ...
    def __init__(self, fname, tokenizer):
        ...
        for i in range(0, len(lines), 3):
            ...
            target_len = np.sum(target_indices != 0)
            polarity = int(polarity)
            # polarity_text = polarity_mapping[polarity]
            text_len = np.sum(text_indices != 0)
            concat_bert_indices = tokenizer.text_to_sequence('[CLS] ' + text + ' [SEP] ' + target + " [SEP]")
            concat_segments_indices = [0] * (text_len + 2) + [1] * (target_len + 1)
            concat_segments_indices = pad_and_truncate(concat_segments_indices, tokenizer.max_seq_len)
            ...
            data = {
                ...
                'text_indices': text_indices,
                'target_indices': target_indices,
                'polarity': polarity,       # polarity_text
                'topic_index': topic_index,
                'text': text,
                'target': target,
                'index': index,
...
if __name__=="__main__":
    ...
    parser.add_argument('--dataset', default='weibo', type=str, help='fewshot_rest,fewshot_mams', required=False)        # fewshot_rest_3way_1
    ...
    parser.add_argument('--num_episode', default=500, type=int, help='try larger number for non-BERT models', required=False)
